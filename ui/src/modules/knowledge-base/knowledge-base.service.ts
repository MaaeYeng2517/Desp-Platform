...
const componentCatalog: ComponentDefinition[] = [
...
const requiredNodeTypes: WorkflowNodeType[] = [
  'chunk',
  'metadata',
  'embedding',
  'vector_database',
...
const nodeOrder: Record<WorkflowNodeType, number> = {
  source_pdf: 0,
  source_docx: 0,
  source_csv: 0,
  source_website: 0,
...
export class KnowledgeBaseService {
  ...
  async create(input: KnowledgeBaseInput): Promise<KnowledgeBaseEntity> {
  ...
  async validate(id: string): Promise<{ id: string; validation: ValidationResult; status: KnowledgeBaseEntity['status'] }> {
    const entity = await this.findOne(id);
  ...
  async run(id: string, request: RunKnowledgeBaseRequest): Promise<PipelineRunResult> {
    const entity = await this.findOne(id);
    const workflow = this.normalizeWorkflow(entity.workflow);
    const metadata = this.normalizeMetadata(entity.metadata);
    const validation = this.validateWorkflowAndMetadata(workflow, metadata);
    ...
    const chunks = request.documents.flatMap((document, documentIndex) =>
      this.createChunks(document, documentIndex, workflow, chunkSize, overlap, dimension),
    );
  ...
  async search(id: string, request: SearchKnowledgeBaseRequest): Promise<{ query: string; results: SearchResult[] }> {
    const entity = await this.findOne(id);
    ...
    const chunks = Array.isArray(entity.chunks) ? (entity.chunks as unknown as ChunkRecord[]) : [];
  ...
  async retrieve(id: string, request: RetrieveKnowledgeBaseRequest): Promise<{ query: string; answer: string; sources: SearchResult[] }> {
  ...
  async createDemo(): Promise<PipelineRunResult> {
  ...
  private normalizeWorkflow(workflow?: Partial<WorkflowDefinition> | null): WorkflowDefinition {
  ...
  private normalizeNode(node: unknown, index: number): WorkflowNode {
    ...
    return {
      ...
      config: typeof candidate.config === 'object' && candidate.config ? (candidate.config as Record<string, unknown>) : { ...definition?.defaultConfig },
  ...
  private isWorkflowNodeType(value: unknown): value is WorkflowNodeType {
    return typeof value === 'string' && Object.keys(nodeOrder).includes(value);
  }

  private normalizeMetadata(metadata?: MetadataInput | null): Record<string, unknown> {
  ...
  private hasPathToAgent(workflow: WorkflowDefinition): boolean {
    const adjacency = new Map<string, string[]>();
    for (const node of workflow.nodes) {
      adjacency.set(node.id, []);
    }
  ...
  private labelForType(type: WorkflowNodeType): string {
    return componentCatalog.find((component) => component.type === type)?.label || type;
  }

  private getNodeConfig(workflow: WorkflowDefinition, type: WorkflowNodeType): Record<string, unknown> {
    return workflow.nodes.find((node) => node.type === type)?.config || {};
  }

  private positiveInteger(value: unknown, fallback: number, maximum: number): number {
  ...
  private embedText(text: string, dimension: number): number[] {
